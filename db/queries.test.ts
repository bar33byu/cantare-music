...
describe("getAllSongs", () => {
  ...
  it("propagates missing-schema errors instead of silently falling back", async () => {
    const schemaError = new Error('column "last_practiced_at" does not exist');
    const failingChain = {
      from: vi.fn(() => {
        throw schemaError;
      }),
    };
    selectSpy.mockReturnValueOnce(failingChain as unknown as ReturnType<typeof makeChain>);

    const { getAllSongs } = await getQueries();
    await expect(getAllSongs()).rejects.toBe(schemaError);
    expect(selectSpy).toHaveBeenCalledOnce();
  });
