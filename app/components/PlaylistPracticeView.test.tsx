...
describe('PlaylistPracticeView', () => {
  ...
  it('exposes Songs, Hands Free, and Listen playlist modes', async () => {
    global.fetch = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ score: 67 }) }) as unknown as typeof fetch;

    render(<PlaylistPracticeView playlist={playlist} onExit={() => undefined} onSelectSong={() => undefined} />);

    expect(await screen.findByTestId('playlist-mode-practice')).toBeInTheDocument();
    expect(screen.getByTestId('playlist-mode-auto')).toHaveTextContent('Hands Free');
    expect(screen.getByTestId('playlist-mode-listen')).toHaveTextContent('Listen');
    expect(screen.queryByTestId('playlist-mode-focus')).not.toBeInTheDocument();
  });
