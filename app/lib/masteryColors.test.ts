...
import { buildMasteryTimelineChunks, getMasteryColor, getMasteryGradientColor, getMasteryLevel, getMasteryTextColor } from "./masteryColors";

describe("getMasteryColor", () => {
  it("maps each rating level to a distinct hue", () => {
    expect(getMasteryColor(0)).toBe("rgb(226, 232, 240)");
    expect(getMasteryColor(20)).toBe("rgb(219, 39, 119)");
    expect(getMasteryColor(40)).toBe("rgb(234, 88, 12)");
    expect(getMasteryColor(60)).toBe("rgb(202, 138, 4)");
    expect(getMasteryColor(80)).toBe("rgb(8, 145, 178)");
  ...
  it("clamps values outside the rating range", () => {
    ...
    expect(getMasteryColor(120)).toBe("rgb(79, 70, 229)");
  });

  it("uses readable text colors across the scale", () => {
    expect(getMasteryTextColor(0)).toBe("rgb(2, 6, 23)");
    expect(getMasteryTextColor(20)).toBe("rgb(255, 255, 255)");
    expect(getMasteryTextColor(80)).toBe("rgb(2, 6, 23)");
    expect(getMasteryTextColor(100)).toBe("rgb(255, 255, 255)");
  });
});
...
describe("getMasteryGradientColor", () => {
  it("interpolates from white to the brand indigo for playlist mastery", () => {
    expect(getMasteryGradientColor(0)).toBe("rgb(255, 255, 255)");
    expect(getMasteryGradientColor(50)).toBe("rgb(167, 163, 242)");
    expect(getMasteryGradientColor(100)).toBe("rgb(79, 70, 229)");
  });

  it("clamps values outside the mastery range", () => {
    expect(getMasteryGradientColor(-20)).toBe("rgb(255, 255, 255)");
    expect(getMasteryGradientColor(120)).toBe("rgb(79, 70, 229)");
  });
});
