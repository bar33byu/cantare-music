...
export async function sendEmailChangeConfirmationEmail({ to, confirmationUrl }: SendEmailChangeData): Promise<void> {
  const apiKey = process.env.RESEND_API_KEY;
  const from = process.env.RESEND_FROM_EMAIL;

  if (!apiKey || !from) {
    throw new Error("RESEND_API_KEY and RESEND_FROM_EMAIL are required to send email-change confirmations");
  }
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from,
      to,
      subject: "Confirm your Cantare email address",
      html: `
        <div style="font-family:Arial,sans-serif;line-height:1.5;color:#111827">
          <h1 style="font-size:22px;margin:0 0 12px">Confirm your Cantare email address</h1>
          <p>Use this secure link to confirm this email address for your Cantare account.</p>
          <p><a href="${confirmationUrl}" style="display:inline-block;background:#4f46e5;color:white;padding:10px 16px;border-radius:6px;text-decoration:none;font-weight:700">Confirm email</a></p>
          <p style="font-size:13px;color:#4b5563">This link expires in 15 minutes and can be used once.</p>
        </div>
      `,
      text: `Confirm your Cantare email address:\n\n${confirmationUrl}\n\nThis link expires in 15 minutes and can be used once.`,
    }),
  });
